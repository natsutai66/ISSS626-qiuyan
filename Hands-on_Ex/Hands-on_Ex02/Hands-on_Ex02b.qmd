...
# 2) Helper: extract fields from KML <Description> (as in Ch 4)
...
# 3) Import & tidy: Singapore subzones (remove Z/M, project to SVY21 EPSG:3414)
...
# Optional: clip out offshore/islands (same as Ch 4)
mpsz_cl <- mpsz_sf %>%
  dplyr::filter(
...
# 4) Import childcare points and project to SVY21
childcare_sf <- sf::st_read(kml_child) %>%
  sf::st_zm(drop = TRUE, what = "ZM") %>%
  sf::st_transform(crs = 3414)

# 5) Convert to spatstat structures for point-process analysis
#    - ppp: point pattern; owin: study window
childcare_ppp <- spatstat.geom::as.ppp(childcare_sf)
sg_owin       <- spatstat.geom::as.owin(mpsz_cl)

# 6) Clip points to Singapore window (ppp within owin)
childcareSG_ppp <- childcare_ppp[sg_owin]

# 7) Create planning-area subsets used later in Ch 5 (G/F functions)
pg <- mpsz_cl %>% dplyr::filter(PLN_AREA_N == "PUNGGOL")
tm <- mpsz_cl %>% dplyr::filter(PLN_AREA_N == "TAMPINES")
ck <- mpsz_cl %>% dplyr::filter(PLN_AREA_N == "CHOA CHU KANG")
jw <- mpsz_cl %>% dplyr::filter(PLN_AREA_N == "JURONG WEST")
...
# 8) (Optional but handy) rescale to kilometres for distance plots
childcareSG_ppp_km  <- spatstat.geom::rescale.ppp(childcareSG_ppp, 1000, "km")
childcare_pg_ppp_km <- spatstat.geom::rescale.ppp(childcare_pg_ppp, 1000, "km")
childcare_tm_ppp_km <- spatstat.geom::rescale.ppp(childcare_tm_ppp, 1000, "km")
childcare_ck_ppp_km <- spatstat.geom::rescale.ppp(childcare_ck_ppp, 1000, "km")
childcare_jw_ppp_km <- spatstat.geom::rescale.ppp(childcare_jw_ppp, 1000, "km")
```
